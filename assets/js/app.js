...
const topicButtonsGen = () => {
  ...
  for (let i = 0; i < topics.length; i++) {
    let apiButtons = $("<button>"); //  Initiate variable to build APIBUTTONS
    apiButtons
...
const imageCreation = (ajaxData) => {
  let gifImgData = ajaxData.data;
  $("#image-box").empty(); // remove previous images each new request.
  for (let gifIndex = 0; gifIndex < gifImgData.length; gifIndex++) {
    let gifRating = gifImgData[gifIndex].rating;
    let fixedUrl = gifImgData[gifIndex].images.original_still.url;
    let animatedUrl = gifImgData[gifIndex].images.original.url;
    let imageFigure = $("<figure>");
    let imgTag = $("<img>");
